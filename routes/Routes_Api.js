...
const spamLimiter = rateLimit({
    ...
    message: {                  
        status: 429,
        message: "Too many requests. Please try again in 1 minute." // Clearer message
    },
...
router.post('/api/send_message', spamLimiter, (req, res) => { 
    ...
    res.status(200).json({ message: 'Message sending initiated (Rate Limited).' });
});


router.post('/register', spamLimiter, (req, res) => {
    
    res.status(201).json({ message: 'User registration successful (Rate Limited).' });
});
